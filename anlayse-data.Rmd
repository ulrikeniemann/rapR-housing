...
commune_level_data <- read.csv(
  "./datasets/commune_level_data.csv"
)

country_level_data <- read.csv(
  "./datasets/country_level_data.csv"
)
```
...
get_laspeyeres <- function(dataset){
  ...
  # search in string -> quo(column-name) to prevent from evaluation
  group_var <- if(grepl("commune", which_dataset)){
                quo(locality)
              } else {
                NULL
              }
  # evaluate by !!group_var 
  # (read https://dplyr.tidyverse.org/articles/programming.html)
  dataset %>%
    group_by(!!group_var) %>%
...
```{r, results = "asis"}
# see literate programming: create knit_childs for every commune
